...
const STUDY_DAYS=[1,2,3,4,5]; // Tue-Sat, JS day indexes
...
function App(){
 ...
 const [fillDay,setFillDay]=useState(1);
 ...
 const openFill=()=>{setFillParity("odd");setFillDay(1);setFillData({});setFillDraft([]);setFillOpen(true)};
 const fillDays=[1,2,3,4,5];
 const fillDayName=DAYS[fillDay];
 ...
 const startFill=(p:Parity)=>{setFillParity(p);setFillDay(1);setFillData({});loadFillDay(p,1,{});};
 const saveFillDay=()=>{const next={...fillData,[String(fillDay)]:fillDraft.filter(x=>x.text.trim())};setFillData(next);if(fillDay<5){const nd=fillDay+1;setFillDay(nd);loadFillDay(fillParity,nd,next)}return next};
